...
import com.ccjeng.iwish.presenter.impl.IntroducePresenter;
import com.ccjeng.iwish.view.adapter.IntroduceAdapter;
...
public class IntroduceActivity extends BaseActivity implements OnStartDragListener {
    ...
    private IIntroducePresenter presenter;
    ...
    private void initRecyclerListener(){
        ...
        swipeToDismissTouchHelper = new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(
            ...
            public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
                presenter.deleteIntroduceById(mIntroduces.get(viewHolder.getAdapterPosition()).getId());
                mIntroduces.remove(viewHolder.getAdapterPosition());
    ...
    public void showData(List<Introduce> introduces) {
        this.mIntroduces = introduces;
        mAdapter = new IntroduceAdapter(mIntroduces, fontSize);

        mAdapter.setOnItemClickListener(new IntroduceAdapter.OnItemClickListener() {
            @Override
    ...
    private void showAddDialog() {
        ...
        dialog.setListener(new AddDialog.OnAddClickListener() {
            ...
            public void OnAddClickListener(String name) {
                ...
                introduce.setOrder(mIntroduces.size());

                presenter.addIntroduce(introduce);
                presenter.getAllIntroduce();
    ...
    private void showEditDialog(final int position, final String id, String name) {
        ...
        dialog.setListener(new EditDialog.OnEditClickListener() {
            ...
            public void OnEditClickListener(String name) {
                dialog.dismiss();
                presenter.updateIntroduceById(id, name);
                mIntroduces.get(position).setName(name);
                mAdapter.notifyDataSetChanged();
            }
        });
    }
    ...
    private void startMode(Mode modeToStart){
        ...
        switch (modeToStart) {
            case NORMAL:
                ...
                toolbar.setTitle(getString(R.string.introduce));
    ...
    public void onBackPressed() {
        switch (mode) {
            ...
            case EDIT:
                ...
                break;
            case SORT:
                presenter.saveOrder(mIntroduces);
                startMode(Mode.NORMAL);
                break;
        }
    }
